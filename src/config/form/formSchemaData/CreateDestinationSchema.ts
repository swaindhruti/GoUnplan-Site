...
export const CreateDestinationSchema = z
  .object({
    ...
    commission: z.coerce
      ...
      .optional()
      .default(0),
    maxLimit: z.coerce.number().positive('Participant limit must be a positive number'),
    genderPreference: z.enum(['MALE_ONLY', 'FEMALE_ONLY', 'MIX']).default('MIX'),
    ...
    tripImage: z.string().min(1, 'Trip cover image is required').url('Must be a valid image URL'),
    dayWiseData: z
      .array(
        z.object({
