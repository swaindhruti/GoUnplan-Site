...
import { DollarSign, AlertCircle, BarChart3, TrendingUp, Clock, AlertTriangle } from 'lucide-react';
import {
  AreaChart,
  Area,
  XAxis,
    ...
    <>
      <div className="flex items-center justify-between mb-8">
        <div className="space-y-2">
          ...
          <h2 className="text-3xl font-bold text-slate-900">Revenue Analytics</h2>
          <p className="text-slate-600 font-medium">
            Track your earnings, bookings, and financial performance with precision
          </p>
        </div>
      </div>
      ...
      {revenueLoading ? (
        <div className="flex flex-col items-center justify-center py-16">
          ...
          <span className="text-xl font-semibold text-slate-900">Loading revenue data...</span>
        </div>
      ) : revenueError ? (
        <div className="flex items-center justify-center py-16">
          <div className="text-center">
            ...
            <p className="text-xl font-semibold text-slate-900">{revenueError}</p>
          </div>
        </div>
      ) : revenueData ? (
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
            <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-medium text-emerald-600">Fully Paid</span>
              </div>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-medium text-purple-600">Total Collected</span>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="bg-gradient-to-r from-purple-50 to-indigo-50 p-6 border-b border-slate-200">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-slate-600 font-medium text-base">Time vs Sales Performance</p>
                </div>
            ...
            <div className="p-6">
              {(() => {
                ...
                const chartData =
                  revenueData.monthlyTrend && revenueData.monthlyTrend.length > 0
                    ? revenueData.monthlyTrend.map(item => ({
                        month: `${monthNames[item.monthNum]} ${item.year.toString().slice(-2)}`,
                        fullyPaid: item.fullyPaidRevenue,
                        partiallyPaid: item.partiallyPaidRevenue,
                        pending: item.pendingRevenue,
                        totalRevenue:
                          item.fullyPaidRevenue + item.partiallyPaidRevenue + item.pendingRevenue,
                ...
                const maxValue = Math.max(...chartData.map(item => item.totalRevenue));
                const dynamicDomain = maxValue > 0 ? [0, Math.ceil(maxValue * 1.1)] : [0, 100000];
                      ...
                      <div className="bg-white p-4 rounded-lg shadow-lg border border-gray-200">
                        ...
                        <div className="space-y-1">
                          <p className="text-sm text-emerald-600 font-medium">
                            Fully Paid: ₹{data?.fullyPaid?.toLocaleString() || 0}
                          </p>
                          <p className="text-sm text-blue-600 font-medium">
                            Partially Paid: ₹{data?.partiallyPaid?.toLocaleString() || 0}
                          </p>
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          <defs>
                            ...
                            <linearGradient id="colorPartiallyPaid" x1="0" y1="0" x2="0" y2="1">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-12">
          <div className="text-center">
            ...
            <p className="text-xl font-semibold text-slate-900">No revenue data available</p>
          </div>
        </div>
      )}
    </>
